...
describe("ArenaService referrals + wallet links", () => {
  ...
  it("redeems invite code and stores invited wallet mapping", async () => {
    ...
    const findNetworkSpy = vi
      .spyOn(typed, "findReferralMappingForWalletNetwork")
      .mockResolvedValue(null);
    const ensureSpy = vi
      .spyOn(typed, "ensureWalletInviteMapping")
      .mockResolvedValue(undefined);

    const result = await service.redeemInviteCode({
      wallet: "invited_wallet",
      inviteCode: "duel123456",
    });

    expect(findNetworkSpy).toHaveBeenCalledWith("invited_wallet");
  ...
  it("awards points when market bet tx inspection verifies wallet and amount", async () => {
    const service = createService();
    const typed = service as unknown as {
      getDb: () => unknown;
      solanaOperator: {
    ...
    const inspectMarketBetTransaction = vi.fn().mockResolvedValue({
    ...
    expect(inspectMarketBetTransaction).toHaveBeenCalledWith(
      "market_sig",
      "deadbeef",
    );

...
describe("ArenaService lifecycle", () => {
  ...
  it("processes payout jobs through claim_for and marks as paid", async () => {
    ...
    const typed = service as {
      ...
      claimPayoutJobForProcessing: (jobId: string) => Promise<boolean>;
    ...
    const claimForProcessingSpy = vi
      .spyOn(typed, "claimPayoutJobForProcessing")
    ...
    expect(claimForProcessingSpy).toHaveBeenCalledWith("payout_1");
  ...
  it("queues payout jobs with deterministic ids and conflict-safe inserts", async () => {
    const { current: service } = createService(world);
    const typed = service as {
      getDb: () => unknown;
      queuePayoutJobs: (
      ...
      buildPayoutJobId: (roundId: string, bettorWallet: string) => string;
      persistRoundEvent: (
        eventType: string,
    ...
    const selectMock = vi
      ...
      .mockReturnValueOnce({
        from: vi.fn().mockReturnValue({
          where: vi
            .fn()
            .mockResolvedValue([{ bettorWallet: "bettor_wallet_1" }]),
        }),
      })
      .mockReturnValueOnce({
        from: vi.fn().mockReturnValue({
    ...
    vi.spyOn(typed, "getDb").mockReturnValue(dbMock as never);
    const persistEventSpy = vi
      .spyOn(typed, "persistRoundEvent")
      .mockResolvedValue(undefined);

    await typed.queuePayoutJobs("round_det", "A");
    ...
    expect(inserted[0]).toEqual(
      expect.objectContaining({
        id: typed.buildPayoutJobId("round_det", "bettor_wallet_1"),
        roundId: "round_det",
        bettorWallet: "bettor_wallet_1",
        status: "PENDING",
        attempts: 0,
      }),
    );
    ...
    expect(persistEventSpy).toHaveBeenCalledWith("PAYOUT_JOBS_QUEUED", {
      roundId: "round_det",
      winnerSide: "A",
      jobsCreated: 1,
    });
  ...
  it("marks payout as paid when claim_for reports already claimed", async () => {
    const { current: service } = createService(world);
    const typed = service as {
      getDb: () => unknown;
      processPayoutJobs: () => Promise<void>;
    ...
    const mockSolana = createMockSolanaArenaOperator();
    mockSolana.claimFor.mockRejectedValue(
      new Error("Position already claimed"),
    );
    typed.solanaOperator = mockSolana;
    ...
    vi.spyOn(typed, "getDb").mockReturnValue(dbMock as never);
    vi.spyOn(typed, "claimPayoutJobForProcessing").mockResolvedValue(true);
    const markSpy = vi
      .spyOn(typed, "markPayoutJobResult")
      .mockResolvedValue(true);
  ...
  it("skips payout execution when another worker has already claimed the job lock", async () => {
    const { current: service } = createService(world);
    const typed = service as {
      getDb: () => unknown;
      processPayoutJobs: () => Promise<void>;
    ...
    vi.spyOn(typed, "getDb").mockReturnValue(dbMock as never);
    vi.spyOn(typed, "claimPayoutJobForProcessing").mockResolvedValue(false);
    const markSpy = vi
      .spyOn(typed, "markPayoutJobResult")
      .mockResolvedValue(true);
    ...
    expect(mockSolana.claimFor).not.toHaveBeenCalled();
    expect(markSpy).not.toHaveBeenCalledWith(
      expect.objectContaining({
        id: "payout_3",
        status: "PAID",
      }),

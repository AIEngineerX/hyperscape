...
contract DuelSystem is System {
    ...
    /**
     * @notice Batch record multiple duels (for catch-up sync).
     *
     * @param duelIds Array of duel IDs
     * @param challengerAddresses Array of challenger addresses
     * @param opponentAddresses Array of opponent addresses
     * @param winnerAddresses Array of winner addresses
     * @param challengerStakeValues Array of challenger stake values
     * @param opponentStakeValues Array of opponent stake values
     * @param forfeits Array of forfeit flags
     */
    function recordDuelBatch(
        bytes32[] calldata duelIds,
        address[] calldata challengerAddresses,
        address[] calldata opponentAddresses,
        address[] calldata winnerAddresses,
        uint64[] calldata challengerStakeValues,
        uint64[] calldata opponentStakeValues,
        bool[] calldata forfeits
    ) public {
        uint256 length = duelIds.length;

        for (uint256 i = 0; i < length; i++) {
            bytes32 challengerCharId = PlayerRegistry.getCharacterId(challengerAddresses[i]);
            bytes32 opponentCharId = PlayerRegistry.getCharacterId(opponentAddresses[i]);

            DuelRecord.set(
                duelIds[i],
                challengerAddresses[i],
                opponentAddresses[i],
                winnerAddresses[i],
                challengerCharId,
                opponentCharId,
                challengerStakeValues[i],
                opponentStakeValues[i],
                forfeits[i],
                uint64(block.timestamp)
            );

            // Update stats
            bytes32 winnerCharId = PlayerRegistry.getCharacterId(winnerAddresses[i]);
            address loserAddress = (winnerAddresses[i] == challengerAddresses[i])
                ? opponentAddresses[i]
                : challengerAddresses[i];
            bytes32 loserCharId = PlayerRegistry.getCharacterId(loserAddress);

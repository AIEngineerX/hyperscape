...
library Constants {
    ...
    uint8 constant MAX_EQUIPMENT_SLOT = 10;
    ...
    uint16 constant MAX_BANK_SLOTS = 480;

    /// @notice Maximum bank tabs (0-9 inclusive)
    uint8 constant MAX_BANK_TABS = 10;
    ...
    uint8 constant MAX_TRADE_OFFERS = 28;
    ...
    uint32 constant NOTED_ITEM_OFFSET = 10000;

    /// @notice Reserved item ID for "no item" / empty slot
    uint32 constant EMPTY_ITEM_ID = 0;
    ...
    uint64 constant MAX_GOLD = 2_147_483_647;
    ...
    uint32 constant COMBAT_XP_PER_DAMAGE_X100 = 400;

    /// @notice Hitpoints XP per damage dealt (×100 for precision: 1.33 × 100 = 133)
    uint32 constant HITPOINTS_XP_PER_DAMAGE_X100 = 133;

    /// @notice Controlled style XP per damage per skill (×100: 1.33 × 100 = 133)
    uint32 constant CONTROLLED_XP_PER_DAMAGE_X100 = 133;
}
